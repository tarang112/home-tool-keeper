  ...
  try {
    ...
    for (const item of expiringItems || []) {
      ...
      if (prefs.in_app) {
        ...
        if (!existing || existing.length === 0) {
          const dedupeKey = `${item.id}:expiration:${diffDays}:${userDay.localDateStr}`;
    ...
    for (const item of warrantyItems || []) {
      ...
      if (prefs.in_app) {
        ...
        if (!existing || existing.length === 0) {
          const dedupeKey = `${item.id}:warranty:${diffDays}:${userDay.localDateStr}`;
          const { error: insertError } = await supabase
            .from('notifications')
            .insert({ user_id: item.user_id, item_id: item.id, title, message, dedupe_key: dedupeKey });

          if (!insertError) warrantyNotifications++;
          else if (insertError.code !== '23505') console.error('Error creating warranty notification:', insertError);
        }
      }
